...
window.addEventListener("message", function (event) {
  if (event.data.action == "display") {
    ...
    if (event.data.action == "job") {
      $("#job").html(event.data.value);
    } else if (event.data.action == "bank") {
      $("#bank").html(event.data.value);
    } else if (event.data.action == "society") {
        $("#society").html(event.data.value); 
    }
  }
});
  ...
  $("#playerInventory").droppable({
    drop: function (event, ui) {
      ...
      if (type === "trunk" && itemInventory === "second") {
        ...
        disableInventory(500);
        $.post(
          "http://esx_inventoryhud/TakeFromStorage",
          JSON.stringify({
            item: itemData,
            number: parseInt($("#count").val()),
          })
  ...
  $("#otherInventory").droppable({
    drop: function (event, ui) {
      ...
      if (type === "trunk" && itemInventory === "main") {
        ...
        disableInventory(500);
        $.post(
          "http://esx_inventoryhud/PutIntoStorage",
          JSON.stringify({
            item: itemData,
            number: parseInt($("#count").val()),
          })
